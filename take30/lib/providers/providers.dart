...
class ExploreNotifier extends StateNotifier<ExploreState> {
  ...
  Future<void> selectCategory(String categoryId) async {
    ...
    final scenes = await _api.getPopularScenes(categoryId.isEmpty ? 'all' : category.name);
    state = state.copyWith(scenes: scenes, isLoading: false);
  }

  Future<List<SceneModel>> search(String query) async {
    if (query.isEmpty) {
      return state.scenes;
    }

    return state.scenes.where((scene) {
      return scene.title.toLowerCase().contains(query.toLowerCase()) ||
          scene.category.toLowerCase().contains(query.toLowerCase());
    }).toList();
  }
}
