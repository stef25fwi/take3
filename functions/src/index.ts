...
// ═══════════════════════════════════════════════════════════════════════════
// renderTake60GuidedScene — assembles AI + user segments into a 60s timeline.
//
// V1 stub: validates inputs, normalises segments, computes total duration
// (clamped to 60s) and returns a "preview_ready" payload pointing to the
// last AI segment as the playable preview. The real FFmpeg/MediaConvert step
// will be plugged in once GPU rendering is provisioned.
// ═══════════════════════════════════════════════════════════════════════════
...
interface RenderRequest {
  ...
  aiSegments?: RenderSegmentInput[];
  userSegments?: RenderSegmentInput[];
  markers?: RenderSegmentInput[];
}
...
export const renderTake60GuidedScene = onCall<RenderRequest>(async (req) => {
  ...
  const userByMarker = new Map<string, RenderSegmentInput>();
  ...
  if (markers.length > 0) {
    for (const marker of markers) {
      ...
      const aiSeg = ai.find((s) => s.markerId === id);
      ...
      merged.push({
        ...
        order: Number(marker.order ?? merged.length),
      });
    }
  } else {
    const all: Segment[] = [];
  ...
  let totalDuration = 0;
  for (const seg of merged) totalDuration += seg.durationSeconds;
  ...
  const finalVideoUrl = finalSeg?.videoUrl ?? "";

  const renderId = db.collection("take60_renders").doc().id;
  await db.doc(`take60_renders/${renderId}`).set({
    sceneId,
    userId: auth.uid,
    ...
    finalVideoUrl,
    segments: merged,
    audioRules: data.audioRules ?? null,
    status: finalVideoUrl ? "preview_ready" : "pending_render",
  });
